...
const LandingSection = () => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      const body = JSON.stringify({ firstName, email, type, comment });
      const headers = { 'Content-Type': 'application/json' };
      await submit('https://example.com/api/endpoint', 'POST', body, headers);
      setIsSubmitting(true);
      const { type, message } = await submit(values.firstName);
      setIsSubmitting(false);
      if (type === 'success') {
        resetForm();
        toast({
          title: 'Form submitted successfully!',
          description: `Thank you for your message, ${values.firstName}!`,
          status: 'success',
          duration: 5000,
          isClosable: true,
        });
